...
public class ComplexOresPlugin implements IPlugin {
	...
	public void preInit(ModUtils mod) {
		...
		mod.addSmelting(goldLump, new SimpleItem(Items.gold_ingot));
		...
		Geocraft.diamondOre = oreBlock.addMetaBlock(new SubBlockGeoOre("kimberliteDiamond", new WeightedItem(diamond,
				1, 1, 1), new WeightedItem(diamondNugget, 0, 2, 2)));
		Geocraft.coalOre = oreBlock.addMetaBlock(new SubBlockGeoOre("coalMid", new WeightedItem(coal, 1, 2, 1),
				new WeightedItem(coalNugget, 0, 4, 2)));
		Geocraft.coalDenseOre = oreBlock.addMetaBlock(new SubBlockGeoOre("coalRich", new WeightedItem(coal, 2, 6, 2),
				new WeightedItem(coalNugget, 0, 6, 4)));
		Geocraft.terraQuartzOre = oreBlock.addMetaBlock(new SubBlockGeoOre("terraQuartzOre", new WeightedItem(
				terraQuartz, 2, 4, 1)));
		Geocraft.goldOre = oreBlock.addMetaBlock(new SubBlockGeoOre("goldOre", new WeightedItem(terraQuartz, 1, 4, 1),
				new WeightedItem(goldLump, 1, 3, 1), new WeightedItem(new SimpleItem(Items.gold_nugget, 0), 0, 1, 1)));
		...
		if (GeocraftConfig.complexMines) {
			...
			reg.registerMine(new MineGold(2, 30));//Gold
