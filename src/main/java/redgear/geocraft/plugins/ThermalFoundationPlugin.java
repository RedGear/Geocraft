...
public class ThermalFoundationPlugin implements IPlugin {
	...
	public boolean shouldRun(ModUtils mod, ModState state) {
		return Loader.isModLoaded("ThermalFoundation") && mod.getBoolean("plugins", "ThermalFoundation");
	}
	...
	public void Init(ModUtils inst) {
		if (Loader.isModLoaded("ThermalFoundation") && inst.getBoolean("plugins", "ThermalFoundation")) {
			try {
				...
				reg.registerMine(new MineCylinderComplex("Copper", copper, stone, 4, 16, 6, false));
				reg.registerMine(new MineCylinderComplex("Tin", tin, stone, 4, 16, 6, false));
				reg.registerMine(new MineCylinderComplex("Silver", silver, stone, 4, 10, 6, false));
				reg.registerMine(new MineCylinderComplex("Lead", lead, stone, 4, 8, 4, false));
