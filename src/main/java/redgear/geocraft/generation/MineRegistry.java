...
public class MineRegistry implements IMineRegistry {
	public HashSet<IMine> mines = new HashSet<IMine>();
	public HashSet<NewOre> newOres = new HashSet<NewOre>();
	public HashMap<SimpleItem, Boolean> ignoreOres = new HashMap<SimpleItem, Boolean>(); //If that boolean is true, gen it like normal, false means do nothing.
	...
	public MineRegistry(ModUtils util){
		...
		this.genTrace = util.getBoolean(l1, "genTrace", "Setting this to false will disable all trace ore generation, overriding the individual ore settings");
		this.volumeModifier = (float) util.getDouble(l1, "volumeModifier", "Changes the number of veins in each mine for ALL ores. It is a multiplier. Larger numbers mean more ore, smaller means less.", 1);
		this.densityModifier = (float) util.getDouble(l1, "densityModifier", "Changes the rarities of ALL ores. It is a multiplier. Larger numbers mean further apart , smaller means closer together.", 1);
		...
		this.defaultDensityRate = 4;
	}

	
	/**
	 * Checks for new ores, adds up new ores, generates trace and normal veins if needed
	 * @param blockId
	 * @param blockMeta
			...
			for(NewOre newOre : newOres)
				if(newOre.numberOfVeins > 0 && newOre.numberOfOres > 0)
					if(!checkForDefaults(newOre.block))
						addNewOre(newOre.block, newOre.target, newOre.numberOfVeins, newOre.numberOfOres / newOre.numberOfVeins);
		newOres.clear();
	}
	...
	public void addNewOre(SimpleItem block, SimpleItem target, int veins, int cluster){
		...
		registerMine(new MineCylinder(name, defaultDensityRate * defaultDensityRate, veins * defaultDensityRate, block, target, cluster));
	...
	private class NewOre {
		...
		public NewOre(SimpleItem block, SimpleItem target, int numberOfOres){
			...
			this.numberOfVeins = 1;
		}
		
		public NewOre(int blockId, int blockMeta, int numberOfOres, int targetId){
			this(new SimpleItem(blockId, blockMeta), new SimpleItem(targetId), numberOfOres);
		}
